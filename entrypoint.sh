...
aws s3 cp "$FILENAME_DB".sql.gz "$S3_URI" --quiet
...
aws s3 cp "$FILENAME_PUBLIC".zip "$S3_URI" --quiet
...
then
  ...
  aws s3 cp "$FILENAME_PRIVATE".zip "$S3_URI" --quiet
...
aws s3 cp "$FILENAME_SOURCE".zip "$S3_URI" --quiet
