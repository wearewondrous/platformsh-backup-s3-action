...
platform mount:download -e "$INPUT_PLATFORMSH_ENVIRONMENT" --target "public-temp" --mount "$INPUT_PUBLIC_FILES_PATH" --exclude 'styles' --exclude 'css' --exclude 'js' --exclude 'config_*' --exclude 'translations' -y -q
ls
...
then
  ...
  platform mount:download -e "$INPUT_PLATFORMSH_ENVIRONMENT" --target "private-temp" --mount "$INPUT_PRIVATE_FILES_PATH" --exclude 'twig' -y -q
  ls
